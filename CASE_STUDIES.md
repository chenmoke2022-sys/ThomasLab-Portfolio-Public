...
- **内存增长**：**十几 MB 量级**（随规模近似线性增长）
- **端到端耗时**：从**亚秒级**增长到**数秒级**（10万规模）
...
- **关键样例**：输入 *"Kill the lights"* → 目标 `CMD_LIGHTS_OFF`
  - 触发结果：**命中预期指令**
  - 端到端延迟：Barrier 为**秒级**，Tunneling 为**毫秒级**（约 **3–4 个数量级**差异）
  - 能耗/成本：在同等链路下，Tunneling 模式呈现**数量级级别**节省（具体口径与测量方法在私有证据包）
...
### 结论（脱敏）
- **门控开销**：整体为**个位数毫秒级**；尾部在**十毫秒级**（分位数与完整分布在私有证据包）
